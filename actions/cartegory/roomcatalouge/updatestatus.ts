...
export const update_status_room_catalogue = async (id: bigint | string, newStatus: number) => {
  try {
    const endpoint = `${process.env.NEXT_PUBLIC_API_URL}/api/roomCatalogues/${id}`;
    ...
    const updateEndpoint = `${process.env.NEXT_PUBLIC_API_URL}/api/roomCatalogues/${id}`;
    const payload = {
      name:roomData.name, // Giữ lại thông tin hiện tại
